...
public class PStateCornerMantlingFast : PState
{
    int cornerDir;
    ...
    public PStateCornerMantlingFast(Vector2 entryVelocity)
    {
        ...
        cornerClimbTimer = attr.cornerFastMantleTime;
        player.animator.Play("PlayerCornerQuickMantling");

        float exitSpeedX = entryVelocity.x * attr.fastMantleHorizontalSpeedMultiplier;
        float exitSpeedY = Mathf.Max(entryVelocity.y * attr.fastMantleVerticalSpeedMultiplier, 0.0f);
        exitVelocity = new Vector2(-exitSpeedX, exitSpeedY + 2.0f);
    }
    ...
    public override PState Update()
    {
        ...
        {
            player.transform.position = new Vector3(mantleCorner.position.x + (cornerHandler.fastMantleEndOffsetX * cornerDir * -1), mantleCorner.position.y + cornerHandler.fastMantleEndOffsetY, 0);
